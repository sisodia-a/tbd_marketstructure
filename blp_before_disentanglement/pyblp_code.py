...
uk_logit_problem = pyblp.Problem(uk_logit, uk_product_data)
uk_logit_results = uk_logit_problem.solve()
uk_logit_results 

logit_elasticities = uk_logit_results.compute_elasticities()
logit_mean_elasticities = pd.DataFrame(uk_logit_results.extract_diagonal_means(logit_elasticities))
print(logit_mean_elasticities.mean())
print(logit_mean_elasticities.std())

logit_markups = pd.DataFrame(uk_logit_results.compute_markups())
print(logit_markups.mean())
print(logit_markups.std())
...
baseline_elasticities = uk_results_with_supply.compute_elasticities()
baseline_mean_elasticities = pd.DataFrame(uk_results_with_supply.extract_diagonal_means(baseline_elasticities))
print(baseline_mean_elasticities.mean())
print(baseline_mean_elasticities.std())

baseline_markups = pd.DataFrame(uk_results_with_supply.compute_markups())
...
uk_logit_het1 = (
   pyblp.Formulation('1 + prices + hpwt + mpd + space + Viz1 + Viz4 + Viz5')
)
...
uk_logit_het2 = (
   pyblp.Formulation('1 + prices + hpwt + mpd + space + Viz1*C(Segment) + Viz4*C(Segment) + Viz5*C(Segment)')
)
...
uk_product_formulations_with_supply_het = (
   pyblp.Formulation('1 + hpwt + mpd + space + Viz1 + Viz4 + Viz5', absorb='C(clustering_ids)'),
   pyblp.Formulation('1 + prices + hpwt + mpd + space  + Viz1 + Viz4 + Viz5'),
   pyblp.Formulation('1 + log(hpwt) + log(mpg) + log(space) + trend')
)
...
bmw_w_viz_wo_change_sim = pyblp.Simulation(
        product_formulations = (pyblp.Formulation('1 + hpwt + mpd + space + Viz1 + Viz4 + Viz5', absorb='C(clustering_ids)'),
        pyblp.Formulation('1 + prices + hpwt + mpd + space + Viz1 + Viz4 + Viz5'),
        pyblp.Formulation('1 + log(hpwt) + log(mpg) + log(space) + trend')),
        product_data = uk_product_data,
