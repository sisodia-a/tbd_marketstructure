 ...
 ## Comment for 0-255 to 0-1
    ...
    def compute_losses(self, dataloader, file_type):
        ...
        with trange(len(dataloader)) as t:
            for _, (data, _, wtp1, wtp2, wtp3, wtp4, wtp5, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames) in enumerate(dataloader):
                data = data.to(self.device)
                recon_batch, latent_dist, latent_sample,wtp_pred1, wtp_pred2, wtp_pred3, wtp_pred4, wtp_pred5, visual_attributes = self.model(data, make, makemodel, color, firm, region, price, hp, mpg, mpd, xi_fe, shares, wph)
    ...
    def _compute_q_zCx(self, dataloader):
        ...
        with torch.no_grad():
            for _,(data, label, wtp1, wtp2, wtp3, wtp4, wtp5, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames) in enumerate(dataloader):
                data = data.to(self.device)
                recon_batch, latent_dist, latent_sample, wtp_pred1, wtp_pred2, wtp_pred3, wtp_pred4, wtp_pred5, visual_attributes = self.model(data, make, makemodel, color, firm, region, price, hp, mpg, mpd, xi_fe, shares, wph)
                batch_size, channel, height, width = data.size()
