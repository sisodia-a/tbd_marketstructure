...
makemodel_mds <- function(market) {
  ...
  df_makemodel <- df_filtered[,c(12:ncol(df_filtered))]
...
for(market in 2008:2017){
  # make_forum_data <- make_mds(market)

  forum_data <- makemodel_mds(market)
  ...
  names(forum_data) <- c("clustering_ids","x","y")
      ...
      write.csv(make_mds(market),paste0("./input_validation_test/mds_make_forum_", market,".csv"),row.names = FALSE)
}
