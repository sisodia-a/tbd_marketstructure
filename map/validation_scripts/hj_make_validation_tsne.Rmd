...
make_tsne <- function(market) {
  ...
  df_makes <- df_filtered[,c(5:ncol(df_filtered))]
  ...
  tsne_result <- Rtsne(dist_matrix, perplexity = 5, check_duplicates = FALSE)
  tsne_result <- tsne_result$Y
  
  tsne_coords <- data.frame(f_x = tsne_result[, 1], f_y = tsne_result[, 2], make = unique_makes)
...
for(market in 2008:2017){
  tsne_result <- make_tsne(market)
  write.csv(tsne_result,paste0("./input_validation_test/tsne_make_forum_", market,".csv"),row.names = FALSE)
}
