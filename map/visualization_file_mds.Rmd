...
summary_stats <- uk_product_data %>% group_by(market_ids) %>% summarise(models=n(),Quantity=mean(quantity),price=sum(prices*shares)/sum(shares),hp=sum(hp*shares),weight=sum(wt*shares),hpwt=sum(hpwt*shares)/sum(shares),mpg=sum(mpg*shares)/sum(shares),length=sum(len*shares),width=sum(wid*shares),height=sum(ht*shares),wheelbase=sum(wb*shares),space=sum(space*shares)/sum(shares))

temp <- uk_product_data %>% summarise(models=n(),Quantity=mean(quantity),price=sum(prices*shares)/sum(shares),hp=sum(hp*shares),weight=sum(wt*shares),hpwt=sum(hpwt*shares)/sum(shares),mpg=sum(mpg*shares)/sum(shares),length=sum(len*shares),width=sum(wid*shares),height=sum(ht*shares),wheelbase=sum(wb*shares),space=sum(space*shares)/sum(shares))
...
summary_stats[,3:13] <- round(summary_stats[,3:13],3)
print(summary_stats %>% as.data.frame())
# Table 6: Descriptive Statistics of Structured Data
subset_stats <- summary_stats[, c(1:4, 5:13)] %>% as.data.frame()
names(subset_stats) <- c("Market","No. of Observations","Quantity","Price","HP","Weight","HP/Wt","MPG","Length","Width","Height","Wheelbase","Space")
print(subset_stats)
xtable(subset_stats)
...
calculate_areas <- function(data, x_col, y_col) {
  data %>%
    ...
    summarise(
      ...
      .groups = "drop"  # so we don’t carry grouping forward
    )
}
...
#   # For simplicity, let’s keep alphabetical order or as is.
#   
#   # 5) Create a heatmap
