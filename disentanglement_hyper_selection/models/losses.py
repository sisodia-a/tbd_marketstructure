...
class BtcvaeLoss():
    ...
    def __init__(self, record_loss_every=50, rec_dist="laplace", steps_anneal=0,n_data=1, alpha=1., beta=6., gamma=1., delta=1000.,sup_signal="make",is_mss=True, **kwargs):
    ...
    def __call__(self, data, recon_batch, latent_dist, is_train, storer, wtp_pred, wtp,
                 latent_sample=None):
        ...
        if self.sup_signal == 'make':
           ...
           mse_loss = criterion(wtp_pred.cuda(),wtp.long().cuda()) ## Ankit_GPU - CLASSIFICATION_PROBLEM 
           ...
           wtp = np.reshape(wtp,(x[0],1)) ## WTP_PROBLEM
           mse_loss = nn.MSELoss()(wtp_pred.float().cuda(),wtp.float().cuda())

        rsq = np.sum((wtp_pred.cpu().detach().numpy()-np.sum(wtp.cpu().detach().numpy())/len(wtp.cpu().detach().numpy()))**2)/np.sum((wtp.cpu().detach().numpy()-np.sum(wtp.cpu().detach().numpy())/len(wtp.cpu().detach().numpy()))**2)
