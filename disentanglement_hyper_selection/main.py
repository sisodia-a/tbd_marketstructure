...
def main(args):
    ...
    train_loader, validation_loader, train_loader_unshuffled, train_loader_batch1 = get_dataloaders("cars",batch_size=int(default_config['batch_size']),eval_batchsize=int(default_config['eval_batchsize']),model_name=args.name,sup_signal=args.sup_signal,logger=logger)
...
#    model = VAE(img_size,Encoder_VGG,Decoder_VGG,WTPregression,int(default_config['latent_dim']),default_config['model_type'])
...
#    del evaluator, loss_f, test_loader, trainer, train_loader, optimizer
...
#    viz.reconstruct(samples, location, brand, circa, movement, diameter, material, timetrend, filenames, size=size)
...
#    viz.gif_traversals(samples[:int(default_config['n_cols'])*2, ...], n_latents=int(default_config['n_rows']))
