 ...
 ## Comment for 0-255 to 0-1
    ...
    def compute_losses(self, dataloader, file_type):
        ...
        with trange(len(dataloader)) as t:
            for _, (data, _, wtp, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames) in enumerate(dataloader):
                data = data.to(self.device)
                recon_batch, latent_dist, latent_sample,wtp_pred, visual_attributes = self.model(data, make, makemodel, color, firm, region, price, hp, mpg, mpd, xi_fe)
    ...
    def _compute_q_zCx(self, dataloader):
        ...
        with torch.no_grad():
            for _,(data, label, wtp, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames) in enumerate(dataloader):
                data = data.to(self.device)
                recon_batch, latent_dist, latent_sample, wtp_pred, visual_attributes = self.model(data, make, makemodel, color, firm, region, price, hp, mpg, mpd, xi_fe)
                batch_size, channel, height, width = data.size()
