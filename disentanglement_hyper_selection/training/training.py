...
# Below 4 lines for comment for 0-255 for 0-1
    ...
    def _train_epoch(self, data_loader, storer, epoch):
        ...
        with trange(len(data_loader), **kwargs) as t:
            for _, (data, _, wtp, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames, hpwt, space, wt, length, wid, ht, wb, xi_fe) in enumerate(data_loader):
                iter_loss, rec_loss, mi_loss, tc_loss, dw_kl_loss, mse_loss, rsq, latent_kl = self._train_iteration(data, storer, wtp, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames, hpwt, space, wt, length, wid, ht, wb, xi_fe, epoch+1) 
    ...
    def _train_iteration(self, data, storer, wtp, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames, hpwt, space, wt, length, wid, ht, wb, xi_fe, epoch):
        """
        ...
        recon_batch, latent_dist, latent_sample, wtp_pred, visual_attributes = self.model(data, make, makemodel, color, firm, region, price, hp, mpg, mpd, hpwt, space, wt, length, wid, ht, wb, xi_fe)
    ...
    def _compute_q_zCx(self, dataloader):
        ...
        logvar = []
        with torch.no_grad():
            for _,(data, label, wtp, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames, hpwt, space, wt, length, wid, ht, wb, xi_fe) in enumerate(dataloader):
                data = data.to(self.device)
                recon_batch, latent_dist, latent_sample, wtp_pred, visual_attributes = self.model(data, make, makemodel, color, firm, region, price, hp, mpg, mpd, hpwt, space, wt, length, wid, ht, wb, xi_fe)
                batch_size, channel, height, width = data.size()
    ...
    def compute_losses(self, dataloader, epoch):
        ...
        with trange(len(dataloader)) as t:
            for _, (data, _, wtp, make, makemodel, color, firm, region, price, hp, mpg, mpd, filenames, hpwt, space, wt, length, wid, ht, wb, xi_fe) in enumerate(dataloader):
                data = data.to(self.device)
                recon_batch, latent_dist, latent_sample,wtp_pred, visual_attributes = self.model(data, make, makemodel, color, firm, region, price, hp, mpg, mpd, hpwt, space, wt, length, wid, ht, wb, xi_fe)
                loss, rec_loss, mi_loss, tc_loss, dw_kl_loss, mse_loss, rsq, latent_kl = self.loss_f(data, recon_batch, latent_dist, self.model.training, storer, wtp_pred, wtp, latent_sample=latent_sample)
                batch_size, channel, height, width = data.size()
