...
udr_beta_inner <- function(beta,seed,m) {
  focal_df <- read.csv(paste0("s",seed,"b",beta,"m",m,"_total.csv"))
  ...
  for(i in 1:10)
  {
    if(i!=seed)
    {
      comparison_df <- read.csv(paste0("s",i,"b",beta,"m",m,"_total.csv"))
...
write.csv(df_udr,"df_udr.csv",row.names=FALSE)
